...
def minimize_all_windows():
    pyautogui.hotkey('win', 'm')  # Presses Win + M to minimize all windows
...
def take_screenshot():
    ...
    file_name = f"C:\\Users\\Revanth\\OneDrive\\Pictures\\Screenshots_{timestamp}.png"
...
with mp_hands.Hands(
    ...
    min_tracking_confidence=0.125) as hands:
            ...
            if all_hands:
                for hand_landmarks in all_hands:
                    ...
                    # Clicking operation
                        ...
                        if distance < 0.8:  # 0.01 is verified limiting distance for the click
                            pyautogui.click()

                    # Minimize all windows gesture( except ring finger and pinky all fingers up )
                    elif finger_states == [1, 1, 1, 0, 0] and hand_orientation == 'right':
                        minimize_all_windows()

                    # Take screenshot when index finger (8) and middle finger (12) of the left hand are close
                    if hand_orientation == 'left' and finger_states[1] == 1 and finger_states[2] == 1:
                        ...
                        if calculate_distance(index_tip, middle_tip) < 0.03:  # Adjust threshold as needed
                            take_screenshot()
